...
import { aha, jasperSmiech } from "@components/discordEmotes.js";

import humanizeDuration from "humanize-duration";
import { EmbedBuilder } from "discord.js";

//Fishing exclusive
import { UpdatePass, UpdateRod } from "@apis/Fish/index.js";

import { CurrentTimeOfTheDay, Passes, RodUpgrade, RodsList, RodsPricing } from "@components/Fish/index.js";
import PriceRolling from "@components/PriceRoll.js";
import { FishCatch, FishPoolCatch, RollNumber } from "@components/Fish/Algorithm/index.js";
import { fishList } from "@components/Fish/data/fishData.js";
...
export default async function commandFish(
    ...
    if (!argumentClean || ' 󠀀'.includes(argumentClean) || ' 󠀀'.includes(args[1])) {
        ...
        const FishData = fishList.fishes[currentTime];
        ...
        const CatchedFish = FishPoolCatch(FishData, userInfo.fishing_rod, FishPoolNumber, fishList.fishes.list_of_fish);

        if (CatchedFish === null) {
            return `<@${discordID}>, błąd podczas losowania ryby.`;
        }

        const fishPrice = PriceRolling(CatchedFish.from_price, CatchedFish.to_price);
